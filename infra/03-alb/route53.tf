...
resource "aws_route53_zone" "fr" {
  ...
  tags = merge(local.common_tags, { Name = "${var.project_name}-hosted-zone-fr" })
}

# Hosted Zone (.com)
resource "aws_route53_zone" "com" {
  name = "one-navigator.com"
  tags = merge(local.common_tags, { Name = "${var.project_name}-hosted-zone-com" })
}
...
resource "aws_route53_record" "aliases" {
  ...
  alias {
    name                   = aws_alb.main.dns_name   # ex: my-alb-123456.eu-west-1.elb.amazonaws.com
    zone_id                = aws_alb.main.zone_id    # zone id de l'ALB
