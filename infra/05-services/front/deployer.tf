  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": [
        "${aws_iam_role.frontend_taskexec.arn}",
...
resource "aws_iam_role" "deployer" {
  name                 = "${var.project_name}-frontend-deployer-${var.region}-${var.environment}"
  description          = "Role used by Github OIDC provider to deploy frontend"
